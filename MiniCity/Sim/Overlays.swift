    ...
    private func updateLandValue() {
        ...
        for cy in 0..<CoarseMap.height {
            for cx in 0..<CoarseMap.width {
                ...
                //
                // 上限を 80 にしていたころは、この折り返しが肝心の場所で切れていた。
                // 都心の街区は density が 490 に達し、本来なら 122 点ぶん効くはずのところを
                // 80 で頭打ちにされて、地価が 202 で止まる。すると住宅の成長点は最良でも
                // 134 点にしかならず、L7 のしきい値 144 に構造的に届かない。
                // 高層タワーの絵が一度も立たなかったのはこれが原因だった。
                // 上限を 140 にすると都心の地価は 252 まで伸び、L9 が立つようになる。
